...
public class AStarEngine {
    ...
    public PositionPart search(World world, Entity me, PositionPart goalState) {
        ...
        fringe.put(initialNode.coordinates(), initialNode);
        while (!fringe.isEmpty()) {

            Node node = getCheapestNode(fringe);
            fringe.remove(node.coordinates());
            visited.put(node.coordinates(), node);
    ...
    private HashMap<String, Node> expandNode(
        ...
        for (PositionPart child : children) {
            ...
            if (visited.containsKey(s.coordinates())) {
                continue;
            }
            ...
            if (fringe.containsKey(s.coordinates()) && s.cost > fringe.get(s.coordinates()).cost) {
                continue;
            }

            successors.put(s.coordinates(), s);
        }

        return successors;
    }

    private ArrayList<PositionPart> getChildren(World world, Entity me, PositionPart state) {
        float x = state.getX();
        float y = state.getY();
...
*/
    ...
    private void addIfValid(World world, Entity me, ArrayList<PositionPart> children, float x, float y) {
        WorldMap worldmap = world.getWorldMap();

        TiledMapTileLayer layer = (TiledMapTileLayer) worldmap.getMap().getLayers().get(0);
        TiledMapTileLayer.Cell cell = layer.getCell((int) x, (int) y);

        if (!cell.getTile().getProperties().containsKey("Blocked")) {
            children.add(new PositionPart(x, y,0));
        }
    }
